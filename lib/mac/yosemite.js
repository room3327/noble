...
nobleBindings.on('kCBMsgId56', function(args) {
  ...
  if (typeof this._peripherals[deviceUuid].services == 'undefined') {
    console.log("--Service create object: " + deviceUuid);
    this._peripherals[deviceUuid].services = {};
  } else {
    console.log("--Services cache hit for " + deviceUuid);
  }
  ...
  if (args.kCBMsgArgServices) {
    for(var i = 0; i < args.kCBMsgArgServices.length; i++) {
     ...
     //   console.log("cache hit for service " + service.uuid);
      }
...
nobleBindings.on('kCBMsgId64', function(args) {
  ...
  // only if the object does not exist yet do we create it.
  if (typeof this._peripherals[deviceUuid].services[serviceStartHandle].characteristics == 'undefined') {
    //console.log("++characteristics create object: " + deviceUuid + " servicestarthandle " + serviceStartHandle);
    this._peripherals[deviceUuid].services[serviceStartHandle].characteristics = {};
  } else {
    //console.log("--characteristics cache hit for " + serviceStartHandle);
  }
  ...
  for(var i = 0; i < args.kCBMsgArgCharacteristics.length; i++) {
    ...
    if (typeof this._peripherals[deviceUuid].services[serviceStartHandle].characteristics[characteristic.uuid] == 'undefined') {
      console.log("++Cache add " + characteristic.uuid);
     ...
     // console.log("--Cache hit the characteristic " + characteristic.uuid);
    } 
  ...
  if (peripheral) {
    for(var i in peripheral.services) {
      if (peripheral.services[i].characteristics &&
       ...
       // console.log("characteristic  " +  peripheral.services[i].characteristics[characteristicHandle].uuid + " emit " + data.toString('hex'));
