...
Noble.prototype.onServicesDiscover = function(peripheralUuid, serviceUuids) {
  ...
  if (peripheral) {
    ...
    // Can it be that all the services have not been discovered and we may only send back a partial list?
    for (var i = 0; i < serviceUuids.length; i++) {
      var serviceUuid = serviceUuids[i];
      var service = null;

      if (typeof this._services[peripheralUuid][serviceUuid] == 'undefined') {
        //  console.log("onServicesDiscover()\n WIPE OUT " + peripheralUuid + " service " + serviceUuid);
          service = new Service(this, peripheralUuid, serviceUuid);
          this._services[peripheralUuid][serviceUuid] = service;
      }

      if (typeof this._characteristics[peripheralUuid][serviceUuid] == 'undefined') {
        //  console.log("WIPE OUT CHARACTERISTIC for service " + serviceUuid);
          this._characteristics[peripheralUuid][serviceUuid] = {};
      }

      if (typeof this._descriptors[peripheralUuid][serviceUuid] == 'undefined') {
        //  console.log("WIPE OUT descriptors for service " + serviceUuid);
    ...
    if (services.length == 0) {
        peripheral.services.map(function(s) {
            services.push(s);
        });
    } else {
...
Noble.prototype.onCharacteristicsDiscover = function(peripheralUuid, serviceUuid, characteristics) {
  ...
  if (service) {
    ...
    for (var i = 0; i < characteristics.length; i++) {
      ...
      // only create a new descriptor object if the one in this slot is not existant.
      if (typeof this._descriptors[peripheralUuid][serviceUuid][characteristicUuid] == 'undefined') {
    ...
    if ((service.characteristics != null) && ( characteristics_.length == 0)) {
        // send back cached characteristics if no new ones were discovered.
        service.characteristics.map(function(c) { characteristics_.push(c); });
    } else {
...
Noble.prototype.onDescriptorsDiscover = function(peripheralUuid, serviceUuid, characteristicUuid, descriptors) {
  ...
  if (characteristic) {
    ...
    if ((characteristics.descriptors != null) && (descriptors_.length == 0)) {
        characteristic.descriptors.map(function(d) { descriptors_.push(d); });
    } else {
